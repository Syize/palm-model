...
!--                In case of land-surfaces, convert fluxes into dynamic units.
                   ...
                   DO  m = 1, surf_lsm%ns
                      ...
                      j = surf_lsm%j(m)
                      k = surf_lsm%k(m)
                      local_pf(i,j,nzb+1) = MERGE( surf_lsm%qsws_agg(m)                            &
...
!--                In case of land- and urban-surfaces, convert fluxes into dynamic units.
                   ...
                   DO  m = 1, surf_lsm%ns
                      ...
                      j = surf_lsm%j(m)
                      k = surf_lsm%k(m)
                      local_pf(i,j,nzb+1) = MERGE( surf_lsm%shf_agg(m) *                           &
...
!--             Quantities of other modules
                IF ( .NOT. found )  THEN
                   CALL module_interface_data_output_2d( av, do2d(av,ivar), found, grid, mode,     &
                                                         local_pf, two_d, flag_nr, nzb_do, nzt_do )
                ENDIF
   ...
   loop1: DO WHILE ( section(is,s_ind) /= -9999  .OR.  two_d )
             ...
             SELECT CASE ( mode )
                ...
                CASE ( 'xy' )
                   ...
                   CALL cpu_log( log_point_s(109), 'data_output_2d_xy', 'start' )
...
!--                This is the non-parallel branch.
                   ...
                   CALL cpu_log( log_point_s(109), 'data_output_2d_xy', 'stop' )
!
...
!--                Hence exit loop of output levels.
                ...
                CASE ( 'xz' )
                   ...
                   CALL cpu_log( log_point_s(110), 'data_output_2d_xz', 'start' )
...
#endif
#endif
                   CALL cpu_log( log_point_s(110), 'data_output_2d_xz', 'stop' )

                CASE ( 'yz' )

                   CALL cpu_log( log_point_s(111), 'data_output_2d_yz', 'start' )
!
...
#endif
#endif
                   CALL cpu_log( log_point_s(111), 'data_output_2d_yz', 'stop' )
