...
! bugfix for processing data from atmosphere-ocean coupled runs (only atmosphere data were
! processed, but twice)
! 
...
!    INTEGER, PARAMETER ::  iwp = 4, spk = SELECTED_REAL_KIND( 6 ), wp = 8
    ...
    INTEGER(iwp) ::  av, danz, i, id, j, k, model, models, nc_stat,            &
                     ...
                     ya_tot, ye, ye_do, ye_tot, yya, yye, za, ze
    ...
    LOGICAL  ::  found, found_ao, nest3d, netcdf_output, netcdf_parallel, netcdf_0, netcdf_1

    REAL(wp) ::  cpu_start_time, cpu_end_time, simulated_time
...
!-- Find out if a coupled or nested run has been carried out
    INQUIRE( FILE='COUPLED_ATMOSPHERE_OCEAN', EXIST=found_ao )
    INQUIRE( FILE='3DNESTING', EXIST=nest3d )
    IF ( found_ao )  THEN
       models = 2
       PRINT*, '    coupled atmosphere-ocean run'
    ELSEIF ( nest3d )  THEN
