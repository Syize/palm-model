...
!--------------------------------------------------------------------------------------------------!
 MODULE plant_canopy_model_mod
    ...
    REAL(wp), DIMENSION(:), ALLOCATABLE ::  bad_s_avg  !< basal area density
    REAL(wp), DIMENSION(:), ALLOCATABLE ::  lad_s_avg  !< leaf area density
    ...
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_latent_flux              !< plant canopy latent heat flux
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_latent_flux_av           !< array for averaging plant canopy latent flux
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_latent_rate              !< plant canopy latent heating rate
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_latent_rate_av           !< array for averaging plant canopy latent heating rate
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_sensible_flux            !< plant canopy sensible heat flux
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_sensible_flux_av         !< array for averaging plant canopy sensible flux
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_sensible_rate            !< plant canopy heating rate
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_sensible_rate_av         !< array for averaging plant canopy sensible heating rate
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_transpiration_rate       !< plant canopy transpiration rate
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_transpiration_rate_av    !< array for averaging plant canopy transpiration rate
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_transpiration_vol        !< plant canopy transpiration rate volume
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_transpiration_vol_av     !< array for averaging plant canopy transpiration rate volume
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_transpiration_vol_pot    !< potential plant canopy transpiration rate volume
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_transpiration_vol_pot_av !< potential array for averaging plant canopy transpiration rate volume
...
!-- Public variables and constants
    PUBLIC bad_s,                                                                                  &
           ...
           pcm_latent_rate_av,                                                                     &
           pcm_sensible_flux,                                                                      &
           pcm_sensible_flux_av,                                                                   &
           pcm_sensible_rate,                                                                      &
           pcm_sensible_rate_av,                                                                   &
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pcm_calc_transpiration_rate( i, j, k, kk, pcbsw, pcblw, pcbtr, pcbtv, pcbtv_pot,       &
                                         pcblh, pcblhfl )
...
!--  Input parameters
     ...
     REAL(wp), INTENT(OUT)    ::  pcbtv              !< transpiration per grid volume and time (m3/m3/s)
     REAL(wp), INTENT(OUT)    ::  pcbtv_pot          !< transpiration per grid volume and time for full soil moisture (m3/m3/s)
...
!--  Variables and parameters for calculation of transpiration rate
     ...
     REAL(wp), PARAMETER ::  m_soil = 0.4_wp       !< volumetric soil water content (needs to adjust or take from LSM) (m3/m3)
     REAL(wp), PARAMETER ::  m_wilt = 0.01_wp      !< wilting point volumetric soil water content (needs to adjust or take from LSM) (m3/m3)
     REAL(wp), PARAMETER ::  m_sat = 0.51_wp       !< saturation volumetric soil water content (needs to adjust or take from LSM) (m3/m3)
     REAL(wp), PARAMETER ::  t2_min = 0.0_wp       !< minimal temperature for calculation of f2 (deg C)
     REAL(wp), PARAMETER ::  t2_max = 40.0_wp      !< maximal temperature for calculation of f2 (deg C)

     REAL(wp) ::  d_fact                !< decoupling factor
     REAL(wp) ::  d_fact_max_water      !< decoupling factor for full soil moisture
     REAL(wp) ::  e_eq                  !< equilibrium evaporation per leaf area and time (kg/m2/s)
     ...
     REAL(wp) ::  f4                    !< soil moisture dependent factor of stomatal conductivity
     REAL(wp) ::  f4_max_water          !< maximal value of f4 for full soil moisture
     REAL(wp) ::  g_b                   !< aerodynamic conductivity (m/s)
     REAL(wp) ::  g_s                   !< stomatal conductivity (m/s)
     REAL(wp) ::  g_s_max_water         !< maximal stomatal conductivity for full soil moisture (m/s)
     REAL(wp) ::  rad                   !< sw radiation per leaf surface unit (W/m2)
     REAL(wp) ::  rswc                  !< residual soil water content
     REAL(wp) ::  sat_press             !< saturation pressure (Pa)
     REAL(wp) ::  sat_press_d           !< saturation pressure derivative with respect to temperature (Pa/deg C)
     REAL(wp) ::  temp                  !< temperature (deg C)
     REAL(wp) ::  v_lad                 !< inverse of leaf area density (m3/m2)
     REAL(wp) ::  vpd                   !< water pressure deficit (Pa)
     REAL(wp) ::  wind_speed            !< wind speed (m/s)


     temp = pt(k,j,i) * exner(k) - degc_to_k
     v_lad = 1.0_wp / MAX( lad_s(kk,j,i), 1.0E-10_wp )
!
!--  Magnus formula (see Ngao, Adam and Saudreau (2017) eq. 5)
...
!--  Stewart(1988))
     f4_max_water = 1.0_wp - 0.041_wp
     f4 = MAX( 0.0_wp, MIN( 1.0_wp - 0.041_wp * EXP( 3.2_wp * rswc ), f4_max_water ) )
!
...
!--  (notation according to Ngao, Adam and Saudreau (2017) and others)
     ...
     g_s_max_water = g_s_max * f1 * f2 * f3 * f4_max_water + 1.0E-10_wp
!
!--  Daudet et al. (1999) eq. 6
     d_fact = ( sat_press_d / gama_psychr + 2.0_wp ) /                                             &
              ( sat_press_d / gama_psychr + 2.0_wp + 2.0_wp * g_b / g_s )
     d_fact_max_water = ( sat_press_d / gama_psychr + 2.0_wp ) /                                   &
                        ( sat_press_d / gama_psychr + 2.0_wp + 2.0_wp * g_b / g_s_max_water )
!
...
!--  Daudet et al. (1999) eq. 5
     ...
     e_imp_pot = e_imp / g_s * g_s_max_water
!
...
!--  dq/dt = E_vol * V_g / (rho_air * V_g)
     pcbtr = evaporation_rate * r_d * pt(k,j,i) * exner(k) / hyp(k)

!
!--  Divide by water density
     pcbtv = evaporation_rate / 1000.0_wp
     pcbtv_pot = evaporation_rate_pot / 1000.0_wp

     pcblh = pcbtr * lv_d_cp  !-- = - dT/dt
     pcblhfl = evaporation_rate * l_v
...
!--       dependencies.
       ...
       CASE ( 'pcm_transpirationvolume' )
          unit = 'm3/m3/s'

       CASE ( 'pcm_transpirationvolume_pot' )
          unit = 'm3/m3/s'
 ...
 SUBROUTINE pcm_3d_data_averaging( mode, variable )
    ...
    IF ( mode == 'allocate' )  THEN
       ...
       SELECT CASE ( TRIM( variable ) )
          ...
          CASE ( 'pcm_heatrate' )
             IF ( .NOT. ALLOCATED( pcm_sensible_rate_av ) )  THEN
                ALLOCATE( pcm_sensible_rate_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
             ENDIF
             pcm_sensible_rate_av = 0.0_wp


          CASE ( 'pcm_latentrate' )
             IF ( .NOT. ALLOCATED( pcm_latent_rate_av ) )  THEN
                ALLOCATE( pcm_latent_rate_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
             ENDIF
             pcm_latent_rate_av = 0.0_wp


          CASE ( 'pcm_transpirationrate' )
             IF ( .NOT. ALLOCATED( pcm_transpiration_rate_av ) )  THEN
                ALLOCATE( pcm_transpiration_rate_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
             ENDIF
             pcm_transpiration_rate_av = 0.0_wp


          CASE ( 'pcm_transpirationvolume' )
               IF ( .NOT. ALLOCATED( pcm_transpiration_vol_av ) )  THEN
                  ALLOCATE( pcm_transpiration_vol_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
               ENDIF
               pcm_transpiration_vol_av = 0.0_wp

          CASE ( 'pcm_transpirationvolume_pot' )
               IF ( .NOT. ALLOCATED( pcm_transpiration_vol_pot_av ) )  THEN
                  ALLOCATE( pcm_transpiration_vol_pot_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
               ENDIF
               pcm_transpiration_vol_pot_av = 0.0_wp

          CASE ( 'pcm_heatflux' )
             IF ( .NOT. ALLOCATED( pcm_sensible_flux_av ) )  THEN
                ALLOCATE( pcm_sensible_flux_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
             ENDIF
    ...
    ELSE IF ( mode == 'sum' )  THEN
       ...
       SELECT CASE ( TRIM( variable ) )
          ...
          CASE ( 'pcm_heatflux' )
             IF ( ALLOCATED( pcm_sensible_flux_av ) )  THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      IF ( pch_index_ji(j,i) /= 0 )  THEN
                         DO  k = 0, pch_index_ji(j,i)
                            pcm_sensible_flux_av(k,j,i) = pcm_sensible_flux_av(k,j,i) +            &
                                                          pcm_sensible_flux(k,j,i)
                         ENDDO
                      ENDIF
                   ENDDO
                ENDDO
          ...
          CASE ( 'pcm_heatrate' )
             IF ( ALLOCATED( pcm_sensible_rate_av ) )  THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      IF ( pch_index_ji(j,i) /= 0 )  THEN
                         DO  k = 0, pch_index_ji(j,i)
                            pcm_sensible_rate_av(k,j,i) = pcm_sensible_rate_av(k,j,i) +            &
                                                          pcm_sensible_rate(k,j,i)
                         ENDDO
                      ENDIF
                   ENDDO
                ENDDO
          ...
          CASE ( 'pcm_latentflux' )
             IF ( ALLOCATED( pcm_latent_flux_av ) ) THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      IF ( pch_index_ji(j,i) /= 0 )  THEN
                         DO  k = 0, pch_index_ji(j,i)
                            pcm_latent_flux_av(k,j,i) = pcm_latent_flux_av(k,j,i) +                &
                                                        pcm_latent_flux(k,j,i)
                         ENDDO
                      ENDIF
                   ENDDO
                ENDDO
          ...
          CASE ( 'pcm_latentrate' )
             IF ( ALLOCATED( pcm_latent_rate_av ) )  THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      IF ( pch_index_ji(j,i) /= 0 )  THEN
                         DO  k = 0, pch_index_ji(j,i)
                            pcm_latent_rate_av(k,j,i) = pcm_latent_rate_av(k,j,i) +                &
          ...
          CASE ( 'pcm_transpirationrate' )
             IF ( ALLOCATED( pcm_transpiration_rate_av ) )  THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      IF ( pch_index_ji(j,i) /= 0 )  THEN
                         DO  k = 0, pch_index_ji(j,i)
                            pcm_transpiration_rate_av(k,j,i) = pcm_transpiration_rate_av(k,j,i) +  &
            ...
            CASE ( 'pcm_transpirationvolume' )
               IF ( ALLOCATED( pcm_transpiration_vol_av ) )  THEN
                  DO  i = nxl, nxr
                     DO  j = nys, nyn
                        IF ( pch_index_ji(j,i) /= 0 )  THEN
                           DO  k = 0, pch_index_ji(j,i)
                              pcm_transpiration_vol_av(k,j,i) = pcm_transpiration_vol_av(k,j,i) +  &
                                                                pcm_transpiration_vol(k,j,i)
                           ENDDO
                        ENDIF
                     ENDDO
                  ENDDO
            ...
            CASE ( 'pcm_transpirationvolume_pot' )
               IF ( ALLOCATED( pcm_transpiration_vol_pot_av ) )  THEN
                  DO  i = nxl, nxr
                     DO  j = nys, nyn
                        IF ( pch_index_ji(j,i) /= 0 )  THEN
                           DO  k = 0, pch_index_ji(j,i)
                              pcm_transpiration_vol_pot_av(k,j,i) =                                &
                                                             pcm_transpiration_vol_pot_av(k,j,i) + &
                                                             pcm_transpiration_vol_pot(k,j,i)
                           ENDDO
                        ENDIF
                     ENDDO
                  ENDDO
...
!> located.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pcm_data_output_3d( av, variable, found, local_pf, resorted, mask_topography, nzb_do,  &
                                nzt_do )
...
!--    Array local_pf must be preset to 0.0, because otherwise floating point exceptions may
!--    occur in data conversion during raw 3d-output (the whole array is output there, but here data
!--    is not assigned to some of the k-levels).
       CASE ( 'pcm_heatrate' )
          mask_topography = .FALSE.
          IF ( data_output_raw )  local_pf = 0.0_wp
          IF ( av == 0 )  THEN
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_sensible_rate(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ELSE
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_sensible_rate_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
       ...
       CASE ( 'pcm_heatflux' )
          ...
          IF ( av == 0 )  THEN
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_sensible_flux(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ELSE
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_sensible_flux_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
       ...
       CASE ( 'pcm_latentrate' )
          ...
          ELSE
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_latent_rate_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
       ...
       CASE ( 'pcm_latentflux' )
          ...
          IF ( av == 0 )  THEN
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_latent_flux(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ELSE
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_latent_flux_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
       ...
       CASE ( 'pcm_transpirationrate' )
          ...
          ELSE
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_transpiration_rate_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
       ...
       CASE ( 'pcm_transpirationvolume' )
          ...
          IF ( av == 0 )  THEN
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_transpiration_vol(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ELSE
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_transpiration_vol_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
       ...
       CASE ( 'pcm_transpirationvolume_pot' )
          ...
          IF ( av == 0 )  THEN
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_transpiration_vol_pot(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ELSE
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_transpiration_vol_pot_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
...
!--  Check for the grid. zpc is zu(nzb:nzb+pch_index)
     SELECT CASE ( TRIM( var ) )
        ...
        CASE ( 'pcm_heatrate', 'pcm_heatflux', 'pcm_bad', 'pcm_lad', 'pcm_transpirationrate',      &
               'pcm_latentrate', 'pcm_latentflux', 'pcm_transpirationvolume',                      &
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pcm_header ( io )
    ...
    CHARACTER (LEN=86) ::  basal_area_density  !<
    ...
    WRITE( io, 1 )  canopy_mode, pch_index, canopy_drag_coeff
...
!
!-- Leaf and basal area density profile
    leaf_area_density = ''
    basal_area_density = ''
    coordinates = ''
    DO  k = 1, pch_index
       ...
       WRITE( coor_chr,'(F7.2)' ) lad_s_avg(k)
       leaf_area_density = TRIM( leaf_area_density ) // ' ' // TRIM( coor_chr )

       WRITE( coor_chr,'(F7.2)' )  bad_s_avg(k)
       basal_area_density = TRIM( basal_area_density ) // ' ' // TRIM( coor_chr )
    ENDDO

    WRITE( io, 4 ) TRIM( coordinates ), TRIM( leaf_area_density ), TRIM( basal_area_density )

!
!-- Print details of the homogeneous leaf area density profile calculation
    IF ( .NOT. TRIM( canopy_mode ) == 'homogeneous' )  RETURN
...
!      Building output strings, starting with surface value
       ...
       WRITE( io, 5 )  TRIM( gradients ), TRIM( slices )

    ELSE

       WRITE( io, 6 ) alpha_lad, beta_lad, lai_beta

    ENDIF

1   FORMAT (/ /' Vegetation canopy model:' / ' ------------------------' //                        &
              '    Canopy mode: ', A / '    Max. canopy height: ',I0,' grid points' /              &
...
4   FORMAT (/ '    Average values of leaf and basal area density:' //                              &
              '       Height level:        ', A /                                                  &
              '       Leaf area density:   ', A, '  m**2/m**3' /                                   &
              '       Basal area density:  ', A, '  m**2/m**3' /)
5   FORMAT ( '    Gradient:            ', A, '  m**2/m**4' /                                       &
             '    Gridpoint:           ', A /)
6   FORMAT ( '    Coefficient alpha: ',F6.2 /                                                      &
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pcm_init
    ...
    INTEGER(iwp) ::  count !< counter for averaging
    ...
    REAL(wp) ::  bad_min  !< minimum BAD value in the model domain, used to perform a check
    REAL(wp) ::  lad_max  !< maximum LAD value in the model domain, used to perform a check
    REAL(wp) ::  lad_min  !< minimum LAD value in the model domain, used to perform a check
    REAL(wp) ::  sum      !< sum for averaging
...
!-- the domain surface.
    SELECT CASE ( TRIM( canopy_mode ) )
       ...
       CASE ( 'homogeneous' )
          ...
          CALL calculate_homogenous_lad

       CASE ( 'read_from_file' )

          CALL read_lad_bad_from_file
...
!-- radiation model.
    ...
    ENDIF
!
!-- Check for negative lad and bad values.
    lad_min = MINVAL( lad_s )
    bad_min = MINVAL( bad_s )
#if defined( __parallel )
    CALL MPI_ALLREDUCE( MPI_IN_PLACE, lad_min, 1, MPI_REAL, MPI_MIN, comm2d, ierr )
    CALL MPI_ALLREDUCE( MPI_IN_PLACE, bad_min, 1, MPI_REAL, MPI_MIN, comm2d, ierr )
#endif
    IF ( lad_min < 0.0_wp  .OR.  bad_min < 0.0_wp )  THEN
       IF ( lad_min < 0.0_wp  .AND.  bad_min < 0.0_wp )  THEN
          message_string = 'lad and bad'
       ELSEIF ( lad_min < 0.0_wp )  THEN
          message_string = 'lad'
       ELSE
          message_string = 'bad'
       ENDIF
       message_string = TRIM( message_string ) // ' out of range'
       CALL message( 'pcm_init', 'PCM0020', 1, 2, 0, 6, 0 )
    ENDIF
...
!
!-- Calculate average leaf and basal area density profiles for grid cells with defined values.
    ALLOCATE( lad_s_avg(pch_index) , bad_s_avg(pch_index) )
    ...
    DO  k = 1, pch_index
       ...
       ENDDO
#if defined( __parallel )
       CALL MPI_ALLREDUCE( MPI_IN_PLACE, sum, 1, MPI_REAL, MPI_SUM, comm2d, ierr )
       CALL MPI_ALLREDUCE( MPI_IN_PLACE, count, 1, MPI_INTEGER, MPI_SUM, comm2d, ierr )
#endif

       IF ( count > 0 )  THEN
          lad_s_avg(k) = sum / count
       ELSE
          lad_s_avg(k) = 0.0_wp
       ENDIF
    ENDDO
...
!
!-- Allocation of arrays pcm_sensible_**, pcm_transpiration_**, and pcm_latent_**
    ALLOCATE( pcm_sensible_flux(0:pch_index,nysg:nyng,nxlg:nxrg) )
    pcm_sensible_flux = 0.0_wp
    ALLOCATE( pcm_sensible_rate(0:pch_index,nysg:nyng,nxlg:nxrg) )
    pcm_sensible_rate = 0.0_wp

    IF ( humidity )  THEN
       ALLOCATE( pcm_transpiration_rate(0:pch_index,nysg:nyng,nxlg:nxrg) )
       pcm_transpiration_rate = 0.0_wp
       ALLOCATE( pcm_transpiration_vol(0:pch_index,nysg:nyng,nxlg:nxrg) )
       pcm_transpiration_vol = 0.0_wp
       ALLOCATE( pcm_transpiration_vol_pot(0:pch_index,nysg:nyng,nxlg:nxrg) )
       pcm_transpiration_vol_pot = 0.0_wp
       ALLOCATE( pcm_latent_rate(0:pch_index,nysg:nyng,nxlg:nxrg) )
       pcm_latent_rate = 0.0_wp
       ALLOCATE( pcm_latent_flux(0:pch_index,nysg:nyng,nxlg:nxrg) )
...
!-- When using the radiation_interactions, canopy heating (pcm_sensible_rate) and plant canopy
...
!--    surface heat flux (shf). Set flux only at upward-facing surfaces. Since the canopy-top
!--    flux is given in W/m2, it need to be converted to kinematic units. The assigned flux
!--    then equals flux times density.
!--    Start with default surface type
       DO  m = 1, surf_def%ns
          i = surf_def%i(m)
          j = surf_def%j(m)
          IF ( cum_lai_hf(0,j,i) /= 0.0_wp  .AND.  surf_def%upward(m) )  THEN
             surf_def%shf(m) = cthf * EXP( -ext_coef * cum_lai_hf(0,j,i) ) * 1.0_wp / c_p
          ENDIF
       ENDDO
!
!--    Natural surfaces
       DO  m = 1, surf_lsm%ns
          i = surf_lsm%i(m)
          j = surf_lsm%j(m)
          IF ( cum_lai_hf(0,j,i) /= 0.0_wp  .AND.  surf_lsm%upward(m) )  THEN
             surf_lsm%shf(m)     = cthf * EXP( -ext_coef * cum_lai_hf(0,j,i) ) * 1.0_wp / c_p
             surf_lsm%shf_agg(m) = cthf * EXP( -ext_coef * cum_lai_hf(0,j,i) ) * 1.0_wp / c_p
          ENDIF
       ENDDO
!
!--    Urban surfaces
       DO  m = 1, surf_usm%ns
          i = surf_usm%i(m)
          j = surf_usm%j(m)
          IF ( cum_lai_hf(0,j,i) /= 0.0_wp  .AND.  surf_usm%upward(m) )  THEN
             surf_usm%shf(m) = cthf * EXP( -ext_coef * cum_lai_hf(0,j,i) ) * 1.0_wp / c_p
          ENDIF
       ENDDO
!
!
...
!--    Within the different canopy layers the plant-canopy heating rate (pcm_sensible_rate) is
...
!--                Multiply with heat capacity and density
                   pcm_sensible_flux(k,j,i) = pcm_sensible_rate(k,j,i) * c_p *                     &
 ...
 SUBROUTINE read_lad_bad_from_file
...
!--       Check if canopy information is consistent to numeric grid.
          ...
          ENDIF
          IF ( ANY( ABS( zlad_tmp(0:leaf_area_density_f%nz-1) - zu(0:leaf_area_density_f%nz-1) ) > &
                    0.001_wp * MINVAL( dz(1:number_dz) ) ) )                                       &
          THEN
             k = 0
             DO  WHILE ( k <= leaf_area_density_f%nz-1 )
                IF ( ABS( zlad_tmp(k) - zu(k) ) > 0.001_wp * MINVAL( dz(1:number_dz) ) )  EXIT
                k = k + 1
             ENDDO
             WRITE( message_string, '(2(A,I4,A,F8.2))' )                                           &
                   'Reading canopy data - vertical coordinate do not match numeric grid at&zlad(', &
                   k, ') = ', zlad_tmp(k), 'with zu(', k, ') = ', zu(k)
...
!--    Read leaf area density - resolved vegetation.
       IF ( check_existence( vars_pids, 'lad' ) )  THEN
          ...
          CALL get_attribute( pids_id, char_fill, leaf_area_density_f%fill, .FALSE., 'lad' )
!
...
!--       Allocate variable for leaf-area density.
          ...
          CALL get_variable( pids_id, 'lad', leaf_area_density_f%var, nxl, nxr, nys, nyn,          &
                             0, leaf_area_density_f%nz-1, nbgp=0 )
...
!--    Read basal area density - resolved vegetation.
       IF ( check_existence( vars_pids, 'bad' ) )  THEN
          ...
          CALL get_attribute( pids_id, char_fill, basal_area_density_f%fill, .FALSE., 'bad' )
!
...
!--       Allocate variable.
          ...
          CALL get_variable( pids_id, 'bad', basal_area_density_f%var, nxl, nxr, nys, nyn,         &
                             0, basal_area_density_f%nz-1, nbgp=0 )
       ELSE
...
!--    Read root area density - resolved vegetation.
       IF ( check_existence( vars_pids, 'root_area_dens_r' ) )  THEN
          ...
          CALL get_attribute( pids_id, char_fill,  root_area_density_lad_f%fill, .FALSE.,          &
                              'root_area_dens_r' )
!
...
!
!--       Allocate variable.
          ALLOCATE( root_area_density_lad_f%var(0:root_area_density_lad_f%nz-1,nys:nyn,nxl:nxr) )

          CALL get_variable( pids_id, 'root_area_dens_r', root_area_density_lad_f%var, nxl, nxr,   &
                             nys, nyn, 0, root_area_density_lad_f%nz-1, nbgp=0 )
       ELSE
       ...
       DO  i = nxl, nxr
          DO  j = nys, nyn
             DO  k = 0, leaf_area_density_f%nz - 1
                IF ( leaf_area_density_f%var(k,j,i) /= leaf_area_density_f%fill )  THEN
       ...
       DO  i = nxl, nxr
          DO  j = nys, nyn
             DO  k = 0, basal_area_density_f%nz - 1
                IF ( basal_area_density_f%var(k,j,i) /= basal_area_density_f%fill )  THEN
...
!--          Please see comment for leaf_area density
 ...
 END SUBROUTINE read_lad_bad_from_file


 SUBROUTINE calculate_homogenous_lad

    INTEGER(iwp) ::  i      !< running index
    INTEGER(iwp) ::  index  !< LAD gradient index
    INTEGER(iwp) ::  j      !< running index
    INTEGER(iwp) ::  k      !< running index
    INTEGER(iwp) ::  kk     !< LAD profile index

    REAL(wp) ::  canopy_height  !< canopy height for lad-profile construction
    REAL(wp) ::  height         !< height for lad-profile construction
    REAL(wp) ::  int_bpdf       !< vertical integral for lad-profile construction
    REAL(wp) ::  gradient       !< gradient for lad-profile construction

    REAL(wp), DIMENSION(:), ALLOCATABLE ::  pre_lad  !< preliminary lad defined on w-grid
...
!-- Meteorology, 106, 437-459).
    IF ( calc_beta_lad_profile )  THEN

       ALLOCATE( pre_lad(0:nz+1) )
       pre_lad = 0.0_wp
...
!--          Preliminary lad profile (defined on w-grid).
             canopy_height = zw(topo_top_ind(j,i,3)+pch_index) - zw(topo_top_ind(j,i,3))
             DO  kk = 0, pch_index
                height = zw(topo_top_ind(j,i,3) + kk) - zw(topo_top_ind(j,i,3))
                pre_lad(kk) = ( height / canopy_height )**( alpha_lad-1.0_wp ) *                   &
                              ( 1.0_wp - ( height / canopy_height ) )**( beta_lad-1.0_wp )
             ENDDO
...
!--          tendencies). Normalization is done such that the sum of the lad multiplied with the
!--          vertical grid spacing yields the leaf area index. lad(0) at the surface is set to zero
!--          by design to indicate that there are cells with non-zero LAD above it.
             lad_s(0,j,i) = 0.0_wp
             int_bpdf = 0.0_wp
             DO  kk = 1, pch_index
                k = topo_top_ind(j,i,3) + kk
                lad_s(kk,j,i) = 0.5 * ( pre_lad(kk-1) + pre_lad(kk) )
                int_bpdf = int_bpdf + lad_s(kk,j,i) * dzw(k)
             ENDDO
             lad_s(:,j,i) = lad_s(:,j,i) / int_bpdf * lai_beta
...
!--    Use vertical gradients for lad-profile construction.
       DO  i = nxlg, nxrg
          DO  j = nysg, nyng
             ...
             lad_s(0,j,i) = lad_surface
             lad_vertical_gradient_level_ind(1) = 0

             DO  kk = 1, pch_index
                k = topo_top_ind(j,i,1) + kk
                height = zu(k) - zu(topo_top_ind(j,i,1))
                IF ( index < 11 )  THEN
                   IF ( lad_vertical_gradient_level(index) < height  .AND.                         &
                        lad_vertical_gradient_level(index) >= 0.0_wp )                             &
                   THEN
                      gradient = lad_vertical_gradient(index)
                      lad_vertical_gradient_level_ind(index) = kk - 1
                      index = index + 1
                   ENDIF
                ENDIF
                IF ( gradient /= 0.0_wp )  THEN
                   IF ( kk /= 1 )  THEN
                      lad_s(kk,j,i) = lad_s(kk-1,j,i) + dzu(k) * gradient
                   ELSE
                      lad_s(kk,j,i) = lad_surface + dzu(k) * gradient
                   ENDIF
                ELSE
                   lad_s(kk,j,i) = lad_s(kk-1,j,i)
                ENDIF
             ENDDO
...
!--          information is used for the HEADER and the RUN_CONTROL file.
 ...
 END SUBROUTINE calculate_homogenous_lad
...
!--------------------------------------------------------------------------------------------------!
    SUBROUTINE pcm_rrd_local_ftn( k, nxlf, nxlc, nxl_on_file, nxrf, nxrc, nxr_on_file, nynf, nync, &
       ...
       SELECT CASE ( restart_string(1:length) )
          ...
          CASE ( 'pcm_sensible_rate_av' )
             IF ( .NOT. ALLOCATED( pcm_sensible_rate_av ) )  THEN
                ALLOCATE( pcm_sensible_rate_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
                pcm_sensible_rate_av = 0.0_wp
             ENDIF
             IF ( k == 1 )  READ( 13 )  tmp_3d2
             pcm_sensible_rate_av(0:pch_index,nysc-nbgp:nync+nbgp,nxlc-nbgp:nxrc+nbgp) =           &
                        tmp_3d2(0:pch_index,nysf-nbgp:nynf+nbgp,nxlf-nbgp:nxrf+nbgp)

          CASE ( 'pcm_sensible_flux_av' )
             IF ( .NOT. ALLOCATED( pcm_sensible_flux_av ) )  THEN
                ALLOCATE( pcm_sensible_flux_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
                pcm_sensible_flux_av = 0.0_wp
             ENDIF
             IF ( k == 1 )  READ( 13 )  tmp_3d2
             pcm_sensible_flux_av(0:pch_index,nysc-nbgp:nync+nbgp,nxlc-nbgp:nxrc+nbgp) =           &
                        tmp_3d2(0:pch_index,nysf-nbgp:nynf+nbgp,nxlf-nbgp:nxrf+nbgp)

          CASE ( 'pcm_latent_rate_av' )
             IF ( .NOT. ALLOCATED( pcm_latent_rate_av ) )  THEN
                ALLOCATE( pcm_latent_rate_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
                pcm_latent_rate_av = 0.0_wp
             ENDIF
             IF ( k == 1 )  READ( 13 )  tmp_3d2
             pcm_latent_rate_av(0:pch_index,nysc-nbgp:nync+nbgp,nxlc-nbgp:nxrc+nbgp) =             &
                        tmp_3d2(0:pch_index,nysf-nbgp:nynf+nbgp,nxlf-nbgp:nxrf+nbgp)

          CASE ( 'pcm_latent_flux_av' )
             IF ( .NOT. ALLOCATED( pcm_latent_flux_av ) )  THEN
                ALLOCATE( pcm_latent_flux_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
                pcm_latent_flux_av = 0.0_wp
             ENDIF
             IF ( k == 1 )  READ( 13 )  tmp_3d2
             pcm_latent_flux_av(0:pch_index,nysc-nbgp:nync+nbgp,nxlc-nbgp:nxrc+nbgp) =             &
                        tmp_3d2(0:pch_index,nysf-nbgp:nynf+nbgp,nxlf-nbgp:nxrf+nbgp)

          CASE ( 'pcm_transpiration_rate_av' )
             IF ( .NOT. ALLOCATED( pcm_transpiration_rate_av ) )  THEN
                ALLOCATE( pcm_transpiration_rate_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
                pcm_transpiration_rate_av = 0.0_wp
             ENDIF
             IF ( k == 1 )  READ( 13 )  tmp_3d2
             pcm_transpiration_rate_av(0:pch_index,nysc-nbgp:nync+nbgp,nxlc-nbgp:nxrc+nbgp) =      &
                        tmp_3d2(0:pch_index,nysf-nbgp:nynf+nbgp,nxlf-nbgp:nxrf+nbgp)

          CASE ( 'pcm_transpiration_vol_av' )
             IF ( .NOT. ALLOCATED( pcm_transpiration_vol_av ) )  THEN
                ALLOCATE( pcm_transpiration_vol_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
                pcm_transpiration_vol_av = 0.0_wp
             ENDIF
             IF ( k == 1 )  READ( 13 )  tmp_3d2
             pcm_transpiration_vol_av(0:pch_index,nysc-nbgp:nync+nbgp,nxlc-nbgp:nxrc+nbgp) =       &
                        tmp_3d2(0:pch_index,nysf-nbgp:nynf+nbgp,nxlf-nbgp:nxrf+nbgp)

          CASE ( 'pcm_transpiration_vol_pot_av' )
             IF ( .NOT. ALLOCATED( pcm_transpiration_vol_pot_av ) )  THEN
                ALLOCATE( pcm_transpiration_vol_pot_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
                pcm_transpiration_vol_pot_av = 0.0_wp
             ENDIF
             IF ( k == 1 )  READ( 13 )  tmp_3d2
             pcm_transpiration_vol_pot_av(0:pch_index,nysc-nbgp:nync+nbgp,nxlc-nbgp:nxrc+nbgp) =   &
                        tmp_3d2(0:pch_index,nysf-nbgp:nynf+nbgp,nxlf-nbgp:nxrf+nbgp)
...
!--    full vertical bounds (bzb:nzt+1) in the restart file and must be re-stored after reading.
       CALL rd_mpi_io_check_array( 'pcm_sensible_rate_av' , found = array_found )
       IF ( array_found )  THEN
          IF ( .NOT. ALLOCATED( pcm_sensible_rate_av ) )  THEN
             ALLOCATE( pcm_sensible_rate_av(nzb:pch_index,nysg:nyng,nxlg:nxrg) )
          ENDIF
          ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
          CALL rrd_mpi_io( 'pcm_sensible_rate_av', tmp_3d )
          pcm_sensible_rate_av = tmp_3d(nzb:pch_index,:,:)
          DEALLOCATE( tmp_3d )
       ENDIF

       CALL rd_mpi_io_check_array( 'pcm_sensible_flux_av' , found = array_found )
       IF ( array_found )  THEN
          IF ( .NOT. ALLOCATED( pcm_sensible_flux_av ) )  THEN
             ALLOCATE( pcm_sensible_flux_av(nzb:pch_index,nysg:nyng,nxlg:nxrg) )
          ENDIF
          ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
          CALL rrd_mpi_io( 'pcm_sensible_flux_av', tmp_3d )
          pcm_sensible_flux_av = tmp_3d(nzb:pch_index,:,:)
          DEALLOCATE( tmp_3d )
       ENDIF

       CALL rd_mpi_io_check_array( 'pcm_latent_rate_av' , found = array_found )
       IF ( array_found )  THEN
          IF ( .NOT. ALLOCATED( pcm_latent_rate_av ) )  THEN
             ALLOCATE( pcm_latent_rate_av(nzb:pch_index,nysg:nyng,nxlg:nxrg) )
          ENDIF
          ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
          CALL rrd_mpi_io( 'pcm_latent_rate_av', tmp_3d )
          pcm_latent_rate_av = tmp_3d(nzb:pch_index,:,:)
          DEALLOCATE( tmp_3d )
       ENDIF

       CALL rd_mpi_io_check_array( 'pcm_latent_flux_av' , found = array_found )
       IF ( array_found )  THEN
          IF ( .NOT. ALLOCATED( pcm_latent_flux_av ) )  THEN
             ALLOCATE( pcm_latent_flux_av(nzb:pch_index,nysg:nyng,nxlg:nxrg) )
          ENDIF
          ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
          CALL rrd_mpi_io( 'pcm_latent_flux_av', tmp_3d )
          pcm_latent_flux_av = tmp_3d(nzb:pch_index,:,:)
          DEALLOCATE( tmp_3d )
       ENDIF

       CALL rd_mpi_io_check_array( 'pcm_transpiration_rate_av' , found = array_found )
       IF ( array_found )  THEN
          IF ( .NOT. ALLOCATED( pcm_transpiration_rate_av ) )  THEN
             ALLOCATE( pcm_transpiration_rate_av(nzb:pch_index,nysg:nyng,nxlg:nxrg) )
          ENDIF
          ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
          CALL rrd_mpi_io( 'pcm_transpiration_rate_av', tmp_3d )
          pcm_transpiration_rate_av = tmp_3d(nzb:pch_index,:,:)
          DEALLOCATE( tmp_3d )
       ENDIF

       CALL rd_mpi_io_check_array( 'pcm_transpiration_vol_av' , found = array_found )
       IF ( array_found )  THEN
          IF ( .NOT. ALLOCATED( pcm_transpiration_vol_av ) )  THEN
       ...
       CALL rd_mpi_io_check_array( 'pcm_transpiration_vol_pot_av' , found = array_found )
       IF ( array_found )  THEN
          IF ( .NOT. ALLOCATED( pcm_transpiration_vol_pot_av ) )  THEN
             ALLOCATE( pcm_transpiration_vol_pot_av(nzb:pch_index,nysg:nyng,nxlg:nxrg) )
          ENDIF
          ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
          CALL rrd_mpi_io( 'pcm_transpiration_vol_pot_av', tmp_3d )
...
!--                Determine topography-top index on scalar-grid
                   DO  k = topo_top_ind(j,i,0)+1, topo_top_ind(j,i,0) + pch_index_ji(j,i)
                      ...
                      tend(k,j,i) = tend(k,j,i) + pcm_sensible_rate(kk,j,i)
                   ENDDO
                ENDDO
             ENDDO
...
!--------------------------------------------------------------------------------------------------!
    SUBROUTINE pcm_wrd_local
       ...
       IF ( TRIM( restart_data_format_output ) == 'fortran_binary' )  THEN
          ...
          IF ( ALLOCATED( pcm_sensible_rate_av ) )  THEN
             CALL wrd_write_string( 'pcm_sensible_rate_av' )
             WRITE( 14 )  pcm_sensible_rate_av
          ENDIF

          IF ( ALLOCATED( pcm_sensible_flux_av ) )  THEN
             CALL wrd_write_string( 'pcm_sensible_flux_av' )
             WRITE( 14 )  pcm_sensible_flux_av
          ENDIF

          IF ( ALLOCATED( pcm_latent_rate_av ) )  THEN
             CALL wrd_write_string( 'pcm_latent_rate_av' )
             WRITE( 14 )  pcm_latent_rate_av
          ENDIF

          IF ( ALLOCATED( pcm_latent_flux_av ) )  THEN
             CALL wrd_write_string( 'pcm_latent_flux_av' )
             WRITE( 14 )  pcm_latent_flux_av
          ENDIF

          IF ( ALLOCATED( pcm_transpiration_rate_av ) )  THEN
             CALL wrd_write_string( 'pcm_transpiration_rate_av' )
             WRITE( 14 )  pcm_transpiration_rate_av
          ENDIF

          IF ( ALLOCATED( pcm_transpiration_vol_av ) )  THEN
            CALL wrd_write_string( 'pcm_transpiration_vol_av' )
            WRITE( 14 )  pcm_transpiration_vol_av
          ENDIF

          IF ( ALLOCATED( pcm_transpiration_vol_pot_av ) )  THEN
            CALL wrd_write_string( 'pcm_transpiration_vol_pot_av' )
            WRITE( 14 )  pcm_transpiration_vol_pot_av
          ENDIF
...
!--       full vertical bounds (bzb:nzt+1) in the restart file and must be re-stored before writing.
          IF ( ALLOCATED( pcm_sensible_rate_av ) )  THEN
             ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
             tmp_3d(nzb:pch_index,:,:)     = pcm_sensible_rate_av
             tmp_3d(pch_index+1:nzt+1,:,:) = 0.0_wp
             CALL wrd_mpi_io( 'pcm_sensible_rate_av', tmp_3d )
             DEALLOCATE( tmp_3d )
          ENDIF
          IF ( ALLOCATED( pcm_sensible_flux_av ) )  THEN
             ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
             tmp_3d(nzb:pch_index,:,:)     = pcm_sensible_flux_av
             tmp_3d(pch_index+1:nzt+1,:,:) = 0.0_wp
             CALL wrd_mpi_io( 'pcm_sensible_flux_av', tmp_3d )
             DEALLOCATE( tmp_3d )
          ENDIF
          IF ( ALLOCATED( pcm_latent_rate_av ) )  THEN
             ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
             tmp_3d(nzb:pch_index,:,:)     = pcm_latent_rate_av
             tmp_3d(pch_index+1:nzt+1,:,:) = 0.0_wp
             CALL wrd_mpi_io( 'pcm_latent_rate_av', tmp_3d )
             DEALLOCATE( tmp_3d )
          ENDIF
          IF ( ALLOCATED( pcm_latent_flux_av ) )  THEN
             ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
             tmp_3d(nzb:pch_index,:,:)     = pcm_latent_flux_av
             tmp_3d(pch_index+1:nzt+1,:,:) = 0.0_wp
             CALL wrd_mpi_io( 'pcm_latent_flux_av', tmp_3d )
             DEALLOCATE( tmp_3d )
          ENDIF
          IF ( ALLOCATED( pcm_transpiration_rate_av ) )  THEN
             ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
             tmp_3d(nzb:pch_index,:,:)     = pcm_transpiration_rate_av
             tmp_3d(pch_index+1:nzt+1,:,:) = 0.0_wp
             CALL wrd_mpi_io( 'pcm_transpiration_rate_av', tmp_3d )
             DEALLOCATE( tmp_3d )
          ENDIF
          IF ( ALLOCATED( pcm_transpiration_vol_av ) )  THEN
            ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
            tmp_3d(nzb:pch_index,:,:)     = pcm_transpiration_vol_av
            tmp_3d(pch_index+1:nzt+1,:,:) = 0.0_wp
            CALL wrd_mpi_io( 'pcm_transpiration_vol_av', tmp_3d )
            DEALLOCATE( tmp_3d )
          ENDIF
          IF ( ALLOCATED( pcm_transpiration_vol_pot_av ) )  THEN
            ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
            tmp_3d(nzb:pch_index,:,:)     = pcm_transpiration_vol_pot_av
            tmp_3d(pch_index+1:nzt+1,:,:) = 0.0_wp
            CALL wrd_mpi_io( 'pcm_transpiration_vol_pot_av', tmp_3d )
            DEALLOCATE( tmp_3d )
          ENDIF
